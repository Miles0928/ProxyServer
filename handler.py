...
class Proxy(BaseHTTPRequestHandler):
    ...
    def do_DIRECT(self):
        ...
        # if self.TLS:
            # try:
                # server_socket.connect_ex(self.Host)
                # print(client_socket, server_socket)
    ...
    def do_FORWARD(self):
        self.maxbuffer = self.maxbuffer*2
        Host_local = self.Host_Forward
        
        client_socket = self.request
        
        try:
            server_socket = socket.socket(*self.Sock_args_v6)
            server_socket.connect(self.Host_local)
        except:
            server_socket = socket.socket(*self.Sock_args)
            server_socket.connect_ex(self.Host_local)
            
        self.Host_in = client_socket.getpeername()[0]
        self.Host_out = server_socket.getsockname()[0]
        print('Forward: {} {} [ {} -->> {} ]'.format(*self.Host, self.Host_in, self.Host_out))
...
class ProxyHandler(Proxy):
    def __init__(self, *args, **kwargs):
        self.proxy_list, self.black_list = config.loadHost()
        super().__init__(*args, **kwargs)
